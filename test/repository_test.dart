...
void main() {
  ...
  final MockFirebaseAuth mockFirebaseAuth =
      MockFirebaseAuth(mockUser: _mockUser);
  final MockFirebaseAuth mockFirebaseAuthWithSignedIn = MockFirebaseAuth(
    mockUser: _mockUser,
    signedIn: true,
  );
  ...
  final AccountRepository accountRepositoryWithSignedIn = AccountRepository(
    auth: mockFirebaseAuthWithSignedIn,
    db: mockFirebaseFirestore,
  );
  ...
  group('AccountRepository', () {
    group('loginWithEmail', () {
      test('Login with Email and Password', () async {
        final user = await accountRepository.loginWithEmail(
            email: _email, password: _password);

        log(user.toString());
        expect(user.user?.uid, _mockUser.uid);
        expect(mockFirebaseAuth.authStateChanges(),
            emitsInOrder([null, isA<User>()]));
        expect(
            mockFirebaseAuth.userChanges(), emitsInOrder([null, isA<User>()]));
      });

      test('Login with Email and Password with error', () async {
        whenCalling(Invocation.method(#signInWithEmailAndPassword, null))
            .on(mockFirebaseAuth)
            .thenThrow(FirebaseAuthException(code: 'veronica'));
        expect(
          () => accountRepository.loginWithEmail(email: '', password: ''),
    ...
    group('signUpWithEmail', () {
      test('Sign up with Email and Password', () async {
        final result = await accountRepository.signUpWithEmail(
            fullname: _displayName, email: _email, password: _password);
      ...
      test('Sign up with Email and Password with error', () async {
        whenCalling(Invocation.method(#createUserWithEmailAndPassword, null))
            .on(mockFirebaseAuth)
            .thenThrow(FirebaseAuthException(code: 'veronica'));
        expect(
          () => accountRepository.signUpWithEmail(
            fullname: '',
            email: '',
            password: '',
          ),
    ...
    group('sendEmailVerification', () {
      test('should send verification email', () async {
        expect(
            accountRepositoryWithSignedIn.sendEmailVerification(), completes);
      });
    });
    ...
    test('Sign out', () async {
      final user = accountRepositoryWithSignedIn.user;

      await accountRepositoryWithSignedIn.signOut();

      expect(accountRepositoryWithSignedIn.user, isNull);
      expect(mockFirebaseAuthWithSignedIn.authStateChanges(),
          emitsInOrder([user, null]));
      expect(mockFirebaseAuthWithSignedIn.userChanges(),
          emitsInOrder([user, null]));
    });
    ...
    group('Delete Account', () {
      test('Delete', () async {
        final user = accountRepositoryWithSignedIn.user;
        expect(
          () => accountRepositoryWithSignedIn.deleteAccount(),
          returnsNormally,
        );
      });

      // test('User.delete can throw exception', () async {
      //   final user = accountRepositoryWithSignedIn;
      //
      ...
      //
      //   expect(
      //     () => mockFirebaseAuthWithSignedIn.currentUser!.delete(),
      //     throwsA(isA<FirebaseAuthException>()),
      //   );
      // });
    });
